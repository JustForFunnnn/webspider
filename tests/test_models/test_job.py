...
class TestJobModel(BaseTestCase):
    ...
    def test_add(self):
        to_add_data_dict = dict(lagou_job_id=10004,
                                city_id=3,
                                company_id=1,
                                title='Python 开发工程师',
                                work_year=5,
    ...
    def test_execute_sql_string(self):
        job_rows = JobModel.execute_sql_string('SELECT id, title FROM job WHERE id = :id', {'id': 1})
        ...
        affect_rows = JobModel.execute_sql_string("UPDATE job SET title = '测试' WHERE id = :id", {'id': 1})
        self.assertEqual(affect_rows, 1)
        job = JobModel.get_by_pk(pk=1)
    ...
    def test_batch_add(self):
        ...
        model_instances = [CityModel(name='你好'),
                           JobModel(title='招聘资深前端工程师', city_id=1, company_id=2, lagou_job_id=100056),
                           JobModel(title='招聘资深中端工程师', city_id=1, company_id=2, lagou_job_id=100055), ]
        ...
        model_instances = [JobModel(title='招聘资深前端工程师', city_id=1, company_id=2, lagou_job_id=100056),
                           JobModel(title='招聘资深中端工程师', city_id=1, company_id=2, lagou_job_id=100055), ]
