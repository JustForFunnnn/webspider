...
def get_jobs_count_from_lagou(city_name, keyword_name):
    """
    爬取拉勾的职位数量
    ...
    headers = generate_http_request_headers(referer='https://www.lagou.com/jobs/list_java?labelWords=&fromSearch=true')
    response_json = requests_post(url=JOB_JSON_URL, params=query_string, data=form_data, headers=headers).json()
