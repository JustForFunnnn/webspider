...
def get_jobs_pagination_from_lagou(lagou_company_id, job_type, page_no=1, is_school_job=False):
    """
    爬取拉勾职位分页数据

    :param lagou_company_id: 拉勾接口使用的公司 id
    :param job_type: 职位类型
    :param page_no: 页码
...
def get_jobs_from_lagou(lagou_company_id, job_type, page_no=1, is_school_job=False):
    """
    爬取拉勾职位数据
    ...
    for job in jobs:
        ...
        job_detail = get_job_detail_from_lagou(lagou_job_id=lagou_job_id)
...
def get_job_detail_from_lagou(lagou_job_id):
    """
    爬取拉勾职位详情页的数据
    ...
    :param lagou_job_id: 拉勾接口使用的职位 id
    :return: 拉勾职位详情页数据
    ...
    if not department:
        logger.error('can not get department by lagou_job_id = {}, html is \n {}'.format(lagou_job_id, response.text))
