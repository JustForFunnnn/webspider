...
def get_companies_from_lagou(city_id=0, finance_stage_id=0, industry_id=0, page_no=1):
    """
    爬取拉勾公司数据
    ...
    for company in companies:
        ...
        company_detail = get_company_detail_from_lagou(lagou_company_id=lagou_company_id)
...
def get_company_detail_from_lagou(lagou_company_id):
    """
    爬取拉勾公司详情页的数据
    ...
    if not sizes:
        logger.error(
            'can not get size by lagou_company_id = {}, html code is \n{}'.format(lagou_company_id, response.text))
