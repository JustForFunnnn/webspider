...
@celery_app.task()
def crawl_lg_jobs_count_task():
    keyword_ids = job_keyword_ctl.get_most_frequently_keyword_ids(limit=1000)
    for keyword_id in keyword_ids:
        crawl_lg_keyword_jobs_count_task.delay(keyword_id)


@celery_app.task()
def crawl_lg_keyword_jobs_count_task(keyword_id):
    cities_name_map = {
        'all_city': u'全国',
        'beijing': u'北京',
        'shanghai': u'上海',
    ...
    for city_name_key, city_name in cities_name_map.items():
        jobs_count_dict[city_name_key] = crawlers.get_jobs_count_from_lg(city_name=city_name,
