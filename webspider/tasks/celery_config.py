...
CELERY_IMPORTS = (  # 指定导入的任务模块
    'webspider.tasks.actor.lg_data',
    'webspider.tasks.actor.lg_jobs_count',
...
CELERYBEAT_SCHEDULE = {
    'crawl_lg_jobs_count_task': {
        'task': 'webspider.tasks.actor.lg_jobs_count.crawl_lg_jobs_count_task',
        'schedule': crontab(hour='01', minute='01', day_of_week='2, 5'),
    },
    'crawl_lg_data_task': {
        'task': 'webspider.tasks.actor.lg_data.crawl_lg_data_task',
...
lg_exchange = Exchange('lg', type='direct')

CELERY_QUEUES = (
    Queue(name='default', exchange=default_exchange, routing_key='default'),
    Queue(name='lg_data', exchange=lg_exchange, routing_key='for_lg_data'),
    Queue(name='lg_jobs_data', exchange=lg_exchange, routing_key='for_lg_jobs_data'),
    Queue(name='lg_jobs_count', exchange=lg_exchange, routing_key='for_lg_jobs_count'),
)

CELERY_ROUTES = {
    'webspider.tasks.actor.lg_data.crawl_lg_job_data_task': {'exchange': 'lg',
                                                             'routing_key': 'for_lg_jobs_data'},
    'webspider.tasks.actor.lg_jobs_count.*': {'exchange': 'lg', 'routing_key': 'for_lg_jobs_count'},
    'webspider.tasks.actor.lg_data.*': {'exchange': 'lg', 'routing_key': 'for_lg_data'},
