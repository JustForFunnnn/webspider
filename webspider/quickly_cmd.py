...
def run_celery_lg_data_worker():
    os.system(
        u'env/bin/celery worker -A webspider.tasks.celery_app -Q lg_data -n lg_data_worker --loglevel=debug')


def run_celery_lg_jobs_data_worker():
    os.system(
        u'env/bin/celery worker -A webspider.tasks.celery_app -Q lg_jobs_data -n lg_jobs_data_worker --loglevel=debug')


def run_celery_lg_jobs_count_worker():
    os.system(
        u'env/bin/celery worker -A webspider.tasks.celery_app -Q lg_jobs_count -n lg_jobs_count_worker --loglevel=debug ')
